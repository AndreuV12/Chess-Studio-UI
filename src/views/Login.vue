<template>
    <div class="grid lg:grid-cols-2 sm:grid-cols-1">
        <div class="flex flex-col justify-center items-center w-full h-screen">
            <div class="shadow-lg border-1 border-gray-200 p-10 rounded-lg">
                <div class="mb-5 sm:mb-8">
                    <h1 class="mb-2 font-semibold text-gray-800 text-title-sm sm:text-title-md">
                        Inicia sessión para acceder al sistema
                    </h1>
                    <p class="text-sm text-gray-500">Introduce tus credenciales para acceder.</p>
                </div>
                <div>
                    <form @submit.prevent="handleSubmit">
                        <div class="space-y-5">
                            <!-- Email -->
                            <div>
                                <label for="username" class="required mb-1.5 block text-sm font-medium text-gray-700"
                                    >Usuario:</label
                                >
                                <input
                            ...
                            <!-- Password -->
                            <div>
                                <label for="password" class="required mb-1.5 block text-sm font-medium text-gray-700">
                                    Contraseña:
                                </label>
                                <div class="relative">
                                    <input
...
<script setup>
    ...
    const handleSubmit = async () => {
        try {
            form.loading = true
            const { access_token } = await auth_api.login({
            ...
            uiStore.showError({ title: 'Error', message: 'Error en el inicio de sesión' })
        } finally {
            form.loading = false
        }
    }
